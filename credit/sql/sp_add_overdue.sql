CREATE DEFINER=`root`@`%` PROCEDURE `small_core`.`sp_add_overdue`(IN IN_CHECK_DATE Date)
BEGIN
	...
	DECLARE cur CURSOR FOR (
		...
		WHERE PLAN_SETTLE_DATE IS NULL 
		-- and 
		-- REPAYMENT_DATE < IN_CHECK_DATE -- NOW() 
		GROUP by PROJECT_ID 
		HAVING max(REPAYMENT_DATE) < IN_CHECK_DATE
	);

	DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = 1; 

-- SELECT IN_CHECK_DATE;
    OPEN cur;
    read_loop:LOOP 
