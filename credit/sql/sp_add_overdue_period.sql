CREATE DEFINER=`root`@`%` PROCEDURE `small_core`.`sp_add_overdue_period`(IN IN_CHECK_DATE Date)
BEGIN
...
-- 		GROUP by ProjectID 
		...
		WHERE RECEIPT_PLAN_DATE < IN_CHECK_DATE -- NOW()
		GROUP by PROJECT_ID
	);
    ...
    read_loop:LOOP 
		...
		-- 最终还款日期距离上个月最后一天的月数
		set v_month_diff = TIMESTAMPDIFF(MONTH,v_ENDDATE,last_day(date_add(IN_CHECK_DATE,interval -1 month)));
		...
		set @i = 1;
	 	while (@i <= v_month_diff) Do -- 循环开始
			...
			TIMESTAMPDIFF(DAY , last_day(date_add(PLAN_REPAY_DT,interval @i month)), IN_CHECK_DATE), 
			FINORGCODE, LOANTYPE, 
			LOANBIZTYPE, BUSINESS_NO, AREACODE, STARTDATE, ENDDATE, 
			CURRENCY, CREDIT_TOTAL_AMT, SHARE_CREDIT_TOTAL_AMT, MAX_DEBT_AMT, GUARANTEEFORM, 
