...
# 通用的
## -- 1. 设置导出时间
-- set @EXPORT_DATE=DATE_FORMAT(NOW() ,'2020-06-01' ); -- '%Y-%m-%d');
set @EXPORT_DATE = DATE_FORMAT('2020-06-01', '%Y-%m-%d');
SELECT  @EXPORT_DATE;
...
## -- tmp_overdue_repo：所有有过逾期的项目，一直跟踪到项目结清，
# 需要上报记录的状态 settle_flag=0，结清后上报一条记录，并置settle_flag=1，不再上报
...
-- 		SELECT PROJECT_ID FROM tmp_overdue_repo
-- -- 		UNION ALL
-- -- 		SELECT DISTINCT APPLY_NO from temp_delay 
-- -- 		UNION ALL
-- -- 		SELECT APPLY_NO  FROM temp_no_repo
-- 		)
-- );


-- 全量
INSERT INTO tmp_overdue_repo( PROJECT_ID, report_date, last_date )
(
	SELECT APPLY_NO, @EXPORT_DATE, @EXPORT_DATE FROM APPLY_ORDER x
	WHERE APPLY_STATUS = 10 AND  NO_SETT_PERIOD > 0
);
-- SELECT COUNT(1) FROM tmp_overdue_repo where settle_flag=0;

# 将申请延期的项目的状态修改为2：settl_flag=2
update tmp_overdue_repo set settle_flag=12  where PROJECT_ID in (
SELECT DISTINCT APPLY_NO from temp_delay 
);

# 将不上报的项目的状态修改为3：settl_flag=3
update tmp_overdue_repo set settle_flag=13  where PROJECT_ID in (
SELECT DISTINCT APPLY_NO from temp_no_repo
);

## 从tmp_overdue_repo中取出尚未结清的项目（settle_flag=0），作为今天要上报的项目清单
...
-- WHERE PROJECT_ID IN (SELECT APPLY_NO FROM APPLY_ORDER WHERE   APPLY_STATUS in (12, 13) and END_DATE > "2020-05-20");
-- 
-- UPDATE tmp_overdue_repo 
-- 	set settle_flag=1
-- -- SELECT * FROM tmp_overdue_repo
-- WHERE PROJECT_ID IN (SELECT APPLY_NO FROM APPLY_ORDER WHERE   NO_SETT_PERIOD =0 and END_DATE > "2020-05-20");
-- 
...
-- 		MAX_OVERDUE_INT, CLASSIFY5, LOAN_STAT, REPAY_MONTH_24_STAT, OVERDRAFT_180DAYS_BAL, 
-- 		LOAN_ACCOUNT_STAT, CUSTNAME, CERTTYPE, CERTNO, CUSTID, 
-- 		BAKE, @EXPORT_DATE FROM tmp_overdue ;
